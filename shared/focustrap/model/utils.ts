...
export function convertToIntOrFallback(stringToConvert: Nullable<string>) {
  if (!stringToConvert) return 0;
  const parsed = parseInt(stringToConvert);
  return parsed ? parsed : 0;
}

export function sortByTabIndex(firstNode: Element, secondNode: Element) {
...
export function getTabIndexOfNode(targetNode: Element) {
